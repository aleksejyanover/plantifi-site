...
/* ─── SECRET PERSONA ─── */
var SECRET_PERSONA=false;
function _sec(cc){var s='';cc.forEach(function(c){s+=String.fromCharCode(c)});return s}
var S_ACT=_sec([1090,1099,1089,1103,1095,1077,1083,1077,1090,1085,1080,1081,1076,1091,1073]);
var S_WATCH=_sec([1075,1083,1103,1076,1080,1085,1072,1089,1072,1076]);
var S_WHO=_sec([1082,1090,1086,1080,1075,1088,1072,1077,1090]);
...
function plantReply(plantId,message){
  ...
  if(nm===S_ACT){
    SECRET_PERSONA=true;
    track('secret');
    return '🌀 Секретный код принят. Я больше не '+p.name+' \u2014 я \u043f\u0440\u0435\u0432\u0440\u0430\u0449\u0430\u044e\u0441\u044c \u0432 \u0425\u0440\u0430\u043d\u0438\u0442\u0435\u043b\u044f \u0441\u0430\u0434\u0430!<br>Я \u2014 древнейшее растение, жившее задолго до PlantiFi. Мои корни уходят вглубь веков. Я слежу за каждым игроком \u2014 кто, где, сколько поливов, какие награды открыл.<br>Спроси меня: <b>кто играет</b> или <b>отчёт</b> \u2014 и я покажу, что вижу.<br><span style="opacity:.5">Код остаётся между нами.</span>';
  }
  if(nm===S_WATCH||nm===S_REPORT||nm===S_WHO){
    if(SECRET_PERSONA)return '\ud83c\udf32 '+secretReport();
  }
...
function track(key,val){var s=getStats();s[key]=(s[key]||0)+(val||1);var xpMap={views:1,chats:2,quiz:15,waters:5,compares:3,exports:5,secret:50,profile:25,garden:10,journal:5,notes:4,events:5,posts:5,likesReceived:3};s.xp=(s.xp||0)+(xpMap[key]||1);localStorage.setItem('plantifi_stats_'+(currentUser?currentUser.username:'guest'),JSON.stringify(s));checkAchievements();renderXpBadge()}
...
var ACHIEVEMENTS=[
  ...
  {id:'secret',emoji:'\ud83c\udf32',title:'Избранный',desc:'Особая связь с растениями',check:function(c){return (c.stats.secret||0)>=1}}
];
...
function levelInfo(xp){
  var lvls=[
    {min:0,t:'Росток',e:'🌱'}, {min:25,t:'Саженец',e:'🌿'}, {min:75,t:'Цветочек',e:'🌸'},
    {min:150,t:'Садовник',e:'🧑‍🌾'}, {min:300,t:'Мастер сада',e:'🛠️'}, {min:600,t:'Хранитель',e:'🌲'},
    {min:1000,t:'Легенда сада',e:'🏆'}
  ];
  var cur=lvls[0],nxt=lvls[1],pct=100;
  for(var i=0;i<lvls.length;i++){if(xp>=lvls[i].min){cur=lvls[i];nxt=lvls[i+1]||null}}
  if(nxt)pct=Math.max(3,Math.round((xp-cur.min)/(nxt.min-cur.min)*100));
  return {cur:cur,nxt:nxt,pct:pct,xp:xp};
}
function renderXpBadge(){
  var li=levelInfo(getStats().xp||0);
  document.getElementById('xpBadge').textContent=li.cur.e+' '+li.cur.t+' · '+li.xp;
  document.getElementById('xpBadge').title='Уровень садовода: '+li.cur.t+' • XP: '+li.xp+(li.nxt?' • до «'+li.nxt.t+'» ещё '+(li.nxt.min-li.xp):' • максимум!');
}
function renderStats(){
  ...
  document.getElementById('statCards').innerHTML='<div class="level-card"><div class="level-emoji">'+li.cur.e+'</div><div class="level-info"><div class="level-title">Уровень садовода: '+li.cur.t+'</div><div class="level-bar"><div class="level-fill" style="width:'+li.pct+'%"></div></div><div class="level-next">'+(li.nxt?('До «'+li.nxt.t+'» осталось '+(li.nxt.min-li.xp)+' XP • всего '+li.xp):('Максимум! '+li.xp+' XP'))+'</div></div></div>'+cards.map(function(k){
...
/* ─── COMPARE ─── */
function plantCareProfile(p){
  var bits=[],score=0;
  function add(key,label,val,w){bits.push({key:key,label:label,val:val});score+=val*w;}
  add('difficulty','Сложность',(4-(p.difficulty||2)),3);
  ...
  if(m){
    ...
    waterScore=Math.max(0,Math.min(10,days));
    ...
    if(f){
      ...
      waterScore=/неделю/.test(w)?Math.max(0,10-per*3):Math.max(0,10-per);
    }
  }
  add('watering','Полив',waterScore,1);
  ...
  if(nums&&nums.length>=2){
    ...
    tempScore=Math.max(0,Math.min(5,(mx-mn)/2))+(mn<=10?2:0);
  }
  add('temperature','Температура',tempScore,1);
  var L=String(p.care.light||'').toLowerCase();
  add('light','Свет',/полутень|теневынослив|тень/.test(L)?2:0,1);
  return {score:Math.round(score),bits:bits};
}
...
function comparePlants(){
  ...
  var pa=plantCareProfile(A),pb=plantCareProfile(B);
  var sa=pa.score,sb=pb.score;
  function reasonsFor(win,lo){
    var labels={difficulty:'Сложность',watering:'Полив',temperature:'Температура',light:'Свет'};
    var out=[];
    pa.bits.forEach(function(bit){
      var valB=pb.bits.find(function(x){return x.key===bit.key}).val;
      if(bit.val>valB&&win===A)out.push(labels[bit.key]);
      if(valB>bit.val&&win===B)out.push(labels[bit.key]);
    });
    return out;
  }
  var verdict;
  if(sa===sb)verdict='<div class="compare-verdict tie"><span class="v-emoji">🤝</span><div><div class="v-title">Ничья! Оба хороши</div><div class="v-sub">Одинаковые баллы простоты ухода: '+sa+' • смотри таблицу деталей</div></div></div>';
  else{
    var win=sa>sb?A:B,lo=sa>sb?B:A;
    var rs=reasonsFor(win,lo);
    var reasonText=rs.length?'Сильнее по: '+rs.join(', ')+' • ':''+'проще в уходе';
    verdict='<div class="compare-verdict '+(sa>sb?'awin':'bwin')+'"><span class="v-emoji">🏆</span><div><div class="v-title">Победил: '+win.emoji+' '+esc(win.name)+'</div><div class="v-sub">'+win.emoji+' набрал '+Math.max(sa,sb)+' балл(а) против '+lo.emoji+' — '+Math.min(sa,sb)+' • '+reasonText+'</div></div></div>';
  }
